...
def postgres_process_rejection(pubsub_msg):
	...
	sel_matches = (
		tbl_matches.select()
			.where(or_(tbl_matches.c.fnc_host_status == MatchesStatus.MATCH_REJECTED, tbl_matches.c.fnc_guest_status == MatchesStatus.MATCH_REJECTED))
	...
	with db.connect() as conn:
		with conn.begin():
			...
			for row in result:
				...
				print(f"setting MATCH {row['db_matches_id']} to {MatchesStatus.MATCH_REJECTED} status")
				...
				print(f"setting HOST {row['fnc_host_id']} to {HostsGuestsStatus.DEFAULT} status")
				...
				print(f"setting GUEST {row['fnc_guest_id']} to {HostsGuestsStatus.DEFAULT} status")
