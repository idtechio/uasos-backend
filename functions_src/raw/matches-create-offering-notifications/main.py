...
def query_acceptance_url(matches_id, accept_value, side):
    ...
    return template_url.format(matches_id=matches_id, accept_value=accept_value.value, side=side.value)
...
def create_paylod_for_guest_get_match_template(matches_id, host_row, guest_row):
    ...
    context = {
        ...
        "url_accept" : query_acceptance_url(matches_id, MatchAcceptanceDecision.ACCEPTED, MatchAcceptanceSide.GUEST),
        "url_reject" : query_acceptance_url(matches_id, MatchAcceptanceDecision.REJECTED, MatchAcceptanceSide.GUEST)
    }

    return create_email_payload(template_id=template_id, context=context, to_emails=create_to_email_element(guest_row['name'], guest_row['email']))
...
def create_paylod_for_host_get_match_template(matches_id, guest_row, host_row):
    ...
    context = {
        ...
        "guest_nationality" : translate_nationality(guest_row['is_ukrainian_nationality']),
        ...
        "url_accept" : query_acceptance_url(matches_id, MatchAcceptanceDecision.ACCEPTED, MatchAcceptanceSide.HOST),
        "url_reject" : query_acceptance_url(matches_id, MatchAcceptanceDecision.REJECTED, MatchAcceptanceSide.HOST)
    }

    return create_email_payload(template_id=template_id, context=context, to_emails=create_to_email_element(host_row['name'], host_row['email']))
#endregion
    ...
    with db.connect() as conn:
        with conn.begin():
            ...
            for match in result:
                sel_guests = (
                    tbl_guests.select()
                        .where(tbl_guests.c.db_guests_id == match["fnc_guests_id"])
                )
                guest_rows = conn.execute(sel_guests)

                sel_hosts = (
                        tbl_hosts.select()
                            .where(tbl_hosts.c.db_hosts_id == match["fnc_hosts_id"])
                    )
