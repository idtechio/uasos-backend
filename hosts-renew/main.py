...
def postgres_renew(db_pool, pubsub_msg):
    ...
    if 'db_hosts_id' not in pubsub_msg.keys():
        raise ValueError(f'key value "db_hosts_id" is missing for RENEW (UPDATE) in "{pubsub_msg}"')
    ...
    stmt = (
        ...
        .where(
            or_(
                tbl_hosts.c.fnc_status == HostsGuestsStatus.FNC_INACTIVE,
                tbl_hosts.c.fnc_status == HostsGuestsStatus.FNC_DISABLED,
            )
        )
        .values(fnc_status=HostsGuestsStatus.MOD_ACCEPTED)
        .returning(tbl_hosts.c.db_hosts_id)
    )
    ...
    with db.connect() as conn:
        with conn.begin():
            ...
            for idx, row in enumerate(result):
                print(f'renewed listing for db_hosts_id={db_hosts_id}')
                processed_rows = idx + 1

            if processed_rows == 0:
                raise ValueError(
                    f'given db_hosts_id={db_hosts_id} is not in FNC_INACTIVE state for RENEW (UPDATE) in "{pubsub_msg}"')
# endregion
